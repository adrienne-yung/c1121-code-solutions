...
function titleCase(title) {
  ...
  for (var i = 0; i < title.length; i++) {
    ...
    if ((title[i] === 'In') || (title[i] === 'For') || (title[i] === 'Of') || (title[i] === 'To') || (title[i] === 'On') || (title[i] === 'And')) {
    ...
    if (title[i] === 'Self-taught') {
      title[i] = 'Self-Taught';
    }
    if ((title[i] === 'The') && (title[i + 1] === 'hat')) {
      title[i] = 'the';
    }
    if ((title[i] === 'The') && (title[i + 1] === 'javascript')) {
      title[i] = 'the';
    }
  }
  return title.join(' ');
